...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	struct PageInfo* pp;
	while(cur_va < end_va && cur_va <= 0xfffff000){
		pp = page_alloc(0);
		if(pp == NULL){
			panic("[region_alloc] page alloc fails.\n");
		}

		if(page_insert(e->env_pgdir, pp, (void*)cur_va, PTE_U | PTE_W)){
			panic("[region_alloc] page insert fails at %p of len %x.\n", va, len);
		}
		cur_va += PGSIZE;
	}
}
